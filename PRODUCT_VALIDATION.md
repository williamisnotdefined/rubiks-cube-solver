...
| `npm run test:e2e` | Passed | Playwright started the native API and web preview, solved shallow and real move-notation flows through HTTP, and checked replay-verified UI results. |
...
| --- | --- | --- |
| Accept a user-provided 3x3 cube state through a web UI. | `apps/web/src/App.tsx` accepts move notation and the API converts that notation into the submitted cube state inside Rust. | `npm run build`, `npm run lint -w @rubiks-cube-solver/web`, `npm run test:e2e`. |
...
| Convert user-facing notation into Rust cubie representation. | Notation parsing and state construction live in Rust; facelets remain an internal API/engine representation, not a UI input. | `cargo test`, `cargo run --quiet -p cube-engine --bin solver_quality_report`, `npm run test:e2e`. |
...
| Let automated tests submit known states, receive solutions, replay them, and verify solved. | `tests/e2e/product-flow.spec.ts` covers shallow solve, real-scramble solve through API, and invalid notation handling. | `npm run test:e2e`. |
...
| Phase 5, Frontend Web | React/Vite app receives move notation, selects solver strategy and limits, displays notation/metrics, and calls the API. | `npm run build`, `npm run lint -w @rubiks-cube-solver/web`, `npm run test:e2e`. |
| Phase 6, E2E Validation | Playwright covers API-backed product flow and failure modes, with native generated artifacts ignored locally. | `npm run test:e2e`. |

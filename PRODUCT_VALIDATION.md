...
| --- | --- | --- |
| Accept a user-provided 3x3 cube state through a web UI. | `apps/web/src/App.tsx` accepts move notation only and sends moves to the Rust HTTP API. Facelet/Kociemba input is not exposed in the interface. | `npm run build`, `npm run lint -w @rubiks-cube-solver/web`, `npm run test:e2e`. |
...
| Phase 5, Frontend Web | React/Vite app receives move notation, selects solver strategy and limits, displays notation/metrics, caps the cube at 350px, and calls the API. | `npm run build`, `npm run lint -w @rubiks-cube-solver/web`, `npm run test:e2e`. |

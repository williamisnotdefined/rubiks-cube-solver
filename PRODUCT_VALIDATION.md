...
| --- | --- | --- |
| Accept a user-provided 3x3 cube state through a web UI. | `web/src/App.tsx` accepts the `Scramble` field only and sends moves to the Rust HTTP API. Facelet/Kociemba input is not exposed in the interface. | `npm run build`, `npm run lint -w @rubiks-cube-solver/web`, `npm run test:e2e`. |
...
| Solve valid states with Rust solver logic exposed through the native HTTP API. | `crates/api` delegates solving to `cube-engine`; `web/src/api/solverClient.ts` is the browser boundary adapter. | `npm run api:test`, `npm run build`, `npm run test:e2e`, `cargo test`. |

...
from vision_ml.replay_scan_sessions import replay_payload, replay_scan_session_exports
from vision_ml.scan_session_export import CANONICAL_FACE_ORDER, load_scan_session_export_file
...
def test_replay_can_target_vision_only(tmp_path: Path) -> None:
    input_path = tmp_path / "scan.json"
    output_dir = tmp_path / "replay"
    input_path.write_text(json.dumps(scan_export()), encoding="utf-8")
    ...
    assert called_urls == ["http://vision.test/analyze-session"]

...
from .scan_session_export import ScanSessionExport, load_scan_session_export_file, scan_session_export_files
...
def replay_scan_session_exports(
    ...
    *,
    ...
    vision_url: str = "http://127.0.0.1:8790",
...
def replay_single_export(
    ...
    *,
    ...
    endpoint = replay_endpoint(api_url=api_url, vision_url=vision_url, vision_only=vision_only)
    metadata = {
        "mode": mode,
        "endpoint": endpoint,
...
def replay_endpoint(*, api_url: str, vision_url: str, vision_only: bool) -> str:
    base_url = vision_url if vision_only else api_url
    route = "/analyze-session" if vision_only else "/scan/solve-session"
...
def main() -> None:
    ...
    parser.add_argument("--vision-url", default="http://127.0.0.1:8790", help="Vision service base URL.")
    parser.add_argument("--vision-only", action="store_true", help="Replay only Vision /analyze-session without solving through the API.")

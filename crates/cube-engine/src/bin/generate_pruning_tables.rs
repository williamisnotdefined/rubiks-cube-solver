...
fn run() -> Result<(), String> {
    ...
    println!(
        "generated {} pruning-table artifacts under {} (phase1_depth={}, phase2_depth={})",
        paths.len(),
...
fn help_text() -> String {
    "usage: generate_pruning_tables [--output DIR] [--max-depth N] [--phase1-max-depth N] [--phase2-max-depth N]\n\nDefault output is crates/cube-engine/pruning-tables. Defaults generate depth-2 deterministic artifacts for fast local smoke checks; raise depths explicitly for deeper local tables."
        .to_owned()
}

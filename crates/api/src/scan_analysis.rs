...
pub async fn solve_scan_session_request(
    ...
    if let Some(message) = validate_scan_session_request(&request) {
        return (
            StatusCode::BAD_REQUEST,
            Json(scan_session_error("invalid_session", message)),
        );
    }

    let scan = match request_scan_session_analysis(state, &request).await {
        Ok(scan) => scan,
        Err((status, response)) => return (status, Json(response)),
    };

    let rescan_faces = rescan_faces_from_session(&scan);
    if !scan.ok || !rescan_faces.is_empty() {
        return (
            StatusCode::OK,
...
fn rescan_faces_from_session(scan: &AnalyzeScanSessionResponse) -> Vec<String> {
    scan.faces
        .iter()
        .filter(|face| !face.analysis.ok || face.analysis.stickers.len() != 9)

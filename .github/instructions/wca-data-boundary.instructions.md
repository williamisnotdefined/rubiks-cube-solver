---
applyTo: "apps/wca-data/**/*.{ts,json,yaml,sql,md},apps/web/src/api/wcaData/**/*.{ts,tsx},apps/web/src/pages/WorldRecordsPage/**/*.{ts,tsx},scripts/wca-data/**/*.mjs,Dockerfile.wca-data,docs/adr/0008-wca-data-api.md,crates/api/src/{routes.rs,tests.rs},deploy/nginx/**/*"
---
...
- A separate `pg-boss` worker schedules import jobs. Import code downloads official TSV exports, stages and transforms rows, atomically activates a dataset, then removes inactive datasets and staging rows without affecting the active dataset.
...
- The production retention policy keeps only the active dataset after a successful import. Rollback requires a database backup or a fresh verified import; destructive cleanup still requires explicit approval and a confirmed target.

...
export function useLiveScanPreview({
  ...
  useEffect(() => {
    ...
    function updateTracking(analysis: AnalyzeScanFaceResponse, capture: CapturedScanImage) {
      ...
      const previousCanAutoFill =
        previousAnalysis !== undefined && isAutoFillReadyAnalysis(previousAnalysis, expectedCenter, gridSize)
      const movement = previousAnalysis === undefined ? Number.POSITIVE_INFINITY : averageAnalysisMovement(previousAnalysis, analysis, gridSize)
      const nextStableFrameCount = canAutoFillCurrent
        ? previousCanAutoFill && movement <= maxTileMovement
          ? stableFrameCountRef.current + 1
          : 1
        : 0
      const nextStatus = liveStatusFromAnalysis(analysis, nextStableFrameCount, expectedCenter, gridSize)
  ...
  }, [enabled, expectedCenter, gridSize, knownCenters, mutateAsync, recordTemporalAnalysis, t, videoRef])
...
function isTrackableAnalysis(analysis: AnalyzeScanFaceResponse, expectedCenter: ScanFaceSymbol, gridSize: 2 | 3): boolean {
  return (
...
function isAutoFillReadyAnalysis(analysis: AnalyzeScanFaceResponse, expectedCenter: ScanFaceSymbol, gridSize: 2 | 3): boolean {
  return (
...
function tileBoxesByIndex(analysis: AnalyzeScanFaceResponse, gridSize: 2 | 3): Map<number, ScanDetectionBox> {
...
function temporalOptions(gridSize: 2 | 3) {
  return { ...defaultTemporalConsensusOptions, gridSize, minTileDetections: gridSize * gridSize }
}

...
export function reverse<T extends Movement | Rotation>(action: T): T {
  ...
  if (action.at(-1) === "'") {
    reversedAction = action.slice(0, -1) as T;
  } else {
    const newAction = `${action}'` as T;
    reversedAction = newAction;
  }
  return reversedAction;
}

/** Translates 3x3 wide notation for a big cube, e.g. on a 6x6 `r` becomes `5r`. */
export function translate<T extends Movement | Rotation>(action: T, cubeType: CubeType): T {
  if (Object.values(Movements.Wide).includes(action as WideMove)) {
    return `${LayerCount[cubeType] - 1}${action}` as T;
  }
  return action;
}
...
export const Movements = Object.freeze({
  ...
  Range: (lower: number, upper: number, baseMove: WideMove | SingleMove): Movement => {
    ...
    const move = `${lower}-${upper}${baseMove}` as Movement;
    if (!isMovement(move)) {
...
export type SingleMove = ValueOf<typeof Movements.Single>;
export type WideMove = ValueOf<typeof Movements.Wide>;
export type TwoMove = ValueOf<typeof Movements.Two>;
export type ThreeMove = ValueOf<typeof Movements.Three>;
export type FourMove = ValueOf<typeof Movements.Four>;
export type FiveMove = ValueOf<typeof Movements.Five>;
export type SixMove = ValueOf<typeof Movements.Six>;
export type RangeMove = `${number}-${number}${SingleMove | WideMove}`;
export type Movement = SingleMove | WideMove | TwoMove | ThreeMove | FourMove | FiveMove | SixMove | RangeMove;
...
export type Rotation = ValueOf<typeof Rotations>;
...
export type Face = ValueOf<typeof Faces>;
...
export type CubeType = ValueOf<typeof CubeTypes>;
...
export function IsRotation(rotation: string): rotation is Rotation {
  return /^([xyz])(\d)?(')?$/.test(rotation);
}

export function isMovement(movement: string): movement is Movement {
